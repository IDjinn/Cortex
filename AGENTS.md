...
- **Guest**: guest chat goes through `POST /api/chat/anonymous` — local providers always, remote only with `X-Provider-Key`. Custom local endpoints (one per provider) live in `stores/localEndpointStore.ts` (`endpoints.Ollama`/`endpoints.LmStudio`; unset = server default) and are sent as `baseUrl` per provider. On login, `authStore.applyAuth` imports the guest snapshot via `POST /api/conversations/import` and clears the guest store on success.
- **Memories**: scoped facts (`Global`/`Project`/`Conversation`) injected into the prompt with a top-K/char budget (`ChatOptions.MemoryTopK`/`MemoryMaxPromptChars`, mirrored client-side in `relevantMemories` from `stores/memoriesStore.ts`). A conversation injects global + its own + its **project chain** (folder and root project) memories — resolved server-side in `MemoryService.RelevantAsync`. Project memories attach via `projectId` (project or folder) and **cascade** when the project is deleted (unlike conversations, which are unfiled). Authed: server-side CRUD (`/api/memories`) via `memoriesStore`, plus bulk `POST /api/memories/bulk-delete` (ids, cap 500) and `POST /api/memories/clear` (scope/projectId/conversationId filter — never filterless); post-turn extraction arrives as an SSE `memoryProposal` event confirmed in `MemoryProposalSheet`. Guest: memories inside `guestStore` (AsyncStorage), injected client-side as a System message in `useChatSession`; no guest extraction and no guest project scope (guests have no projects). `app/memories.tsx` groups by section (Globais / Por projeto with the 2-level tree / Por conversa), long-press starts multi-select bulk delete and each section header has a two-tap "Limpar" clear.
- **Projects (workspace)**: `Project` entity is 2 levels — a root project holds folders (`parentId` self-FK; nesting past level 2 is rejected in `ProjectService`, not by schema). Conversations file into a project or folder via `projectId` (`ConversationResponse.projectId`); PATCH with empty-string `projectId` unfiles, POST creates already filed (unknown ids are ignored server-side, never fail the create). `GET/POST/PATCH/DELETE /api/projects`; deleting a project cascades its folders and **unfiles** conversations (SetNull — chats are never deleted). Authed-only: guests keep a flat sidebar list. Client: `stores/projectsStore.ts` (`buildProjectTree` for the 2-level tree) + `conversationsStore.moveToProject` (which re-fetches projects so sidebar counts stay true).
- **Sidebar workspace**: `components/chat/Sidebar.tsx` renders sections — Projetos (collapsible tree: project → folders → conversations; `+` starts a chat born filed via `onNewChat(projectId)`), Conversas (unfiled, guests' whole list) and Workspace nav rows (Memórias real; Skills/Commands/MCPs/Plugins/Tarefas stubs via `components/screens/ComingSoon.tsx`, tied to roadmap phases). Long-press on a conversation opens `ConversationActionsSheet` (rename / pin / move / delete with two-tap confirm); long-press on a project/folder opens `ProjectActionsSheet` (rename / new folder / delete). Destructive actions always use the two-tap confirm inside the sheet — never `Alert.alert`.
...
- `components/screens/` — screen styles + ComingSoon (outside `app/` so they don't become routes)
- `components/chat/` — Bubble (User/Assistant), ConversationView, ModelPickerSheet, Sidebar (workspace: tree + sections + action sheets), ConversationActionsSheet, ProjectActionsSheet, TypingDots, ReasoningBlock
- `components/code/` — CodeBlock (generic syntax-highlighted code block + copy), highlighter
- `components/settings/` — ProviderKeysCard (todos os provedores do catálogo: BYOK para nuvem + endpoint custom para Ollama/LM Studio, com refresh de modelos ao salvar), UsageCard (estatísticas locais + gráfico mensal + split por provedor), MemoriesCard
...
- `stores/` — Zustand: authStore (com migração guest→conta), conversationsStore, projectsStore (workspace projects), guestStore (conversas + memórias do convidado), modelPrefsStore, keysStore (BYOK device), providersStore (catálogo), localEndpointStore, memoriesStore, settingsStore (idioma + estatísticas)
